    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>Cancel Revision</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                            <li class="breadcrumb-item active">Cancel Revision</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                                        ...
                                        @foreach ($cancels as $value)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <form action="{{ route('revision.update', $value->id) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('PUT')
                                                        <select name="status" id="" class="form-control">
                                                            <option value="0"
                                                                {{ old('status', $value->status) == '0' ? 'selected' : '' }}>
                                                                Pending</option>
                                                            <option value="1"
                                                                {{ old('status', $value->status) == '1' ? 'selected' : '' }}>
                                                                Confirmed</option>
                                                            <option value="2"
                                                                {{ old('status', $value->status) == '2' ? 'selected' : '' }}>
                                                                Cancel</option>
                                                            <option value="3"
                                                                {{ old('status', $value->status) == '3' ? 'selected' : '' }}>
                                                                Trash</option>
                                                        </select>
